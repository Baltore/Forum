...
func Addlike(w http.ResponseWriter, r *http.Request) { //ajout d'un like dans un post
	...
	db, _ = sql.Open("sqlite3", "./forum.db")
	reqdata := "INSERT INTO interaction(postid, uuid, date) VALUES (?, ?, ?)"
	request, _ := db.Prepare(reqdata) // prepare la requete
	_, err = request.Exec(postid, uuid, dt)
	fmt.Print(err)
...
func Addcomment(w http.ResponseWriter, r *http.Request) { //ajout d'un commentaire dans un post
	...
	uuid = uuid[5:]
	var Com Comment
	if err := db.QueryRow("SELECT creationdate, username from users where uuid = ?", uuid).Scan(&Com.Creationdateuser, &Com.Username); err != nil { //requete Database
		return
	}
	dt := time.Now()
	dt.Format("01-02-2006 15:04:05")
	...
	reqdata := "INSERT INTO interaction(postid, uuid, date) VALUES (?, ?, ?)"
	request, _ := db.Prepare(reqdata) // Prepare la requete
	_, _ = request.Exec(post_id, uuid, dt)
	defer request.Close()

	reqdata = "INSERT INTO comments(creator, postid, comment, likes, dislikes, file, creationdateuser, username, date) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)"
	request, _ = db.Prepare(reqdata) // Prepare la requete
	_, _ = request.Exec(uuid, post_id[0], comment, 0, 0, file, Com.Creationdateuser, Com.Username, dt)
	defer request.Close()
}
...
func Getcomments(w http.ResponseWriter, r *http.Request) { //tout les commentaires d'un post
	...
	templ.Comments = nil
	for rows.Next() {
		if err := rows.Scan(&Com.Creator, &Com.Postid, &Com.Comment, &Com.Likes, &Com.Dislikes, &Com.File, &Com.Creationdateuser, &Com.Username, &Com.Date); err != nil {
			return
		}
